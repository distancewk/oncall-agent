...
| mcp | 启用 Spring AI MCP Client，预置 Tavily 和 DBHub stdio server |
...
DBHub 使用 [config/dbhub.toml](../config/dbhub.toml) 作为默认配置。该文件只包含安全的内存 SQLite 示例和 PostgreSQL、MySQL、MariaDB、SQL Server 模板；生产环境应使用仓库外部配置文件并通过 `MCP_DBHUB_CONFIG` 指定路径。
